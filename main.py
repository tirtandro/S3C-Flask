...
def init_db():
    ...
    db.executescript('''
        CREATE TABLE IF NOT EXISTS users (
            ...
            kelas TEXT, tenant_name TEXT, photo TEXT,
        ...
        CREATE TABLE IF NOT EXISTS menus (
            ...
            carbs REAL DEFAULT 0, fat REAL DEFAULT 0, fiber REAL DEFAULT 0,
            is_healthy INTEGER DEFAULT 0, is_available INTEGER DEFAULT 1,
            image_emoji TEXT DEFAULT '🍱', image_file TEXT,
            created_at TEXT DEFAULT (datetime('now')),
        ...
        CREATE TABLE IF NOT EXISTS orders (
            ...
            status TEXT DEFAULT 'pending', total_price REAL DEFAULT 0, notes TEXT,
            created_at TEXT DEFAULT (datetime('now')),
        ...
        CREATE TABLE IF NOT EXISTS order_items (
            ...
            quantity INTEGER DEFAULT 1, subtotal REAL DEFAULT 0,
            FOREIGN KEY (order_id) REFERENCES orders(id),
            FOREIGN KEY (menu_id) REFERENCES menus(id)
        );
        CREATE TABLE IF NOT EXISTS waste_logs (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            student_id INTEGER NOT NULL, menu_id INTEGER,
            waste_level TEXT, waste_reason TEXT, points_earned INTEGER DEFAULT 0,
        ...
        CREATE TABLE IF NOT EXISTS education_posts (
            ...
            category TEXT, image_emoji TEXT DEFAULT '📚', author_id INTEGER,
            created_at TEXT DEFAULT (datetime('now'))
        );
        CREATE TABLE IF NOT EXISTS marketplace_items (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            name TEXT NOT NULL, description TEXT, category TEXT,
            price REAL DEFAULT 0, unit TEXT DEFAULT 'pcs', stock INTEGER DEFAULT 0,
            image_emoji TEXT DEFAULT '♻️', image_file TEXT,
            seller_id INTEGER, is_available INTEGER DEFAULT 1,
            created_at TEXT DEFAULT (datetime('now'))
        );
    ...
    def uid(u): return db.execute('SELECT id FROM users WHERE username=?',(u,)).fetchone()[0]
    ...
    for m in menus:
        db.execute('INSERT INTO menus (tenant_id,name,description,price,category,calories,protein,carbs,fat,fiber,is_healthy,image_emoji) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', m)
    db.commit()

    m1 = db.execute("SELECT id FROM menus WHERE name='Nasi Sayur Tempe'").fetchone()[0]
    m3 = db.execute("SELECT id FROM menus WHERE name='Jus Buah Segar'").fetchone()[0]
    cur = db.execute('INSERT INTO orders (student_id,tenant_id,total_price,status) VALUES (?,?,?,?)',(s1,t1,18000,'done'))
    oid = cur.lastrowid
    db.execute('INSERT INTO order_items (order_id,menu_id,quantity,subtotal) VALUES (?,?,?,?)',(oid,m1,1,8000))
    db.execute('INSERT INTO order_items (order_id,menu_id,quantity,subtotal) VALUES (?,?,?,?)',(oid,m3,1,6000))

    for lvl,rsn,pts in [('habis','Enak!',10),('sedikit','Kenyang',7),('habis','Lapar',10)]:
        db.execute('INSERT INTO waste_logs (student_id,waste_level,waste_reason,points_earned) VALUES (?,?,?,?)',(s1,lvl,rsn,pts))
    db.execute('INSERT INTO waste_logs (student_id,waste_level,waste_reason,points_earned) VALUES (?,?,?,?)',(s2,'setengah','Kurang enak',3))
    ...
    for t,c,cat,em,a in edu:
        db.execute('INSERT INTO education_posts (title,content,category,image_emoji,author_id) VALUES (?,?,?,?,?)',(t,c,cat,em,a))
    ...
    for n,d,c,p,u,s,e,sid in mp:
        db.execute('INSERT INTO marketplace_items (name,description,category,price,unit,stock,image_emoji,seller_id) VALUES (?,?,?,?,?,?,?,?)',(n,d,c,p,u,s,e,sid))
    ...
    for t,u,d,c,a in vids:
        db.execute('INSERT INTO edu_videos (title,youtube_url,description,category,uploader_id,is_published) VALUES (?,?,?,?,?,1)',(t,u,d,c,a))
...
def migrate_db():
    ...
    migrations = [
        ...
        ('users',             'photo',       'TEXT'),
        ('edu_videos',        'is_published','INTEGER DEFAULT 1'),
    ]
    ...
    # Create new tables if missing (edu_videos may not exist in old DBs)
    db.executescript("""
        CREATE TABLE IF NOT EXISTS edu_videos (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            title TEXT NOT NULL, youtube_url TEXT NOT NULL,
...
def dashboard():
    ...
    if role == 'student':
        ...
        top_menus   = query_db('''SELECT m.name,m.image_emoji,SUM(oi.quantity) as total FROM menus m
            JOIN order_items oi ON m.id=oi.menu_id GROUP BY m.id ORDER BY total DESC LIMIT 5''')
...
def tenant_orders():
    ...
    top_menus = query_db('''SELECT m.name,m.image_emoji,SUM(oi.quantity) as total FROM menus m
        JOIN order_items oi ON m.id=oi.menu_id WHERE m.tenant_id=? GROUP BY m.id ORDER BY total DESC LIMIT 5''',[session['user_id']])
